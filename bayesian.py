...
Intuition:
Uses percentages from pre all-star-break, and number of attempts, to construct a pseudo-count prior.
Separates percentages by distance - for dew balls only considers shots from 25-29ft.
'''
def get_priors(playerid, w, k):
    ...
    # k: scaling constant
    new_pc_short = ((shot_distance_data["fg_pc_20-24"] + shot_distance_data["fg_pc_25-29"]) / 2) * k
    new_pc_long = shot_distance_data["fg_pc_25-29"] * k
    ...
    alpha_reg_prior = new_pc_short * last_100_data["att"] * w + epsilon
    # beta prior represents assumed number of misses in the data
    beta_reg_prior = (1-new_pc_short) * last_100_data["att"] * w + 2 * epsilon

    # Prior distribution for dew shots
    alpha_dew_prior = new_pc_long* last_100_data["att"] * pc_long_threes * w + epsilon
    beta_dew_prior = (1-new_pc_long) * last_100_data["att"] * pc_long_threes * w + 2 * epsilon
